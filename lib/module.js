...
var Module = (function (_super) {
    ...
    Module.prototype.__saveNewBill = function (connection, params, callback) {
        ...
        async.waterfall([
            ...
            function calculateTotal(next) {
                var billTotalInfo = billUtils.calculateBillTotal(params.items, [], params.discounts);
    ...
    Module.prototype.getFinancialReport = function (params, callback) {
      ...
      GROUP BY bill.category", [params.fromDate, params.toDate], function (err, rows) {
                cleanup();
                callback(err && new DatabaseError(err), { reports: _.map(rows, function (report) {
                    return report;
                }) });
            });
        });
    ...
    Module.prototype.__getBillHistory = function (connection, params, callback) {
      ...
      WHERE bill.idUser = ? \
      group by bill.idbill", [params.id], function (err, rows) {
                callback(err && new DatabaseError(err), { bills: _.map(rows, function (report) {
                    return report;
                }) });
            });
        });
