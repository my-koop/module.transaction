            ...
            async.waterfall([
                ...
                function (callback) {
                    self.__addBillTransaction(helper.connection(), {
                        ...
                        callback(err || (!result.success && new ApplicationError(null, {}, "Cannot create transaction for bill %d with amount %d", params.idBill, params.amount)), result);
                    });
                },
        ...
        async.waterfall([
            ...
            function (callback) {
                ...
                if (!params.archiveBill) {
                    this.__addBillTransaction(mysqlHelper.connection(), {
                        idBill: idBill,
                        amount: params.total
                    }, function (err, result) {
                        callback(err || (!result.success && new ApplicationError(null, {}, "Cannot create transaction for bill %d with amount %d", idBill, params.total)));
                    });
                    return;
                }
                callback(null, 0);
    ...
    Module.prototype.__addBillTransaction = function (connection, params, callback) {
        this.__addTransaction(connection, { amount: params.amount }, function (err, res) {
            ...
            connection.query("INSERT INTO bill_transaction SET idBill = ?, idTransaction = ?", [params.idBill, res.idTransaction], function (err, row) {
                ...
                callback(err && new DatabaseError(err, "Error inserting into bill_transaction"), result);
            });
        });
    };
    return Module;
