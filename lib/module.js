...
var Module = (function (_super) {
    ...
    Module.prototype.__saveNewBill = function (connection, params, callback) {
        ...
        async.waterfall([
          ...
          idEvent = ?, \
          category = ?, \
          closedDate = " + closedDate, [params.total, params.notes, idUser, idEvent, params.category], function (err, res) {
    ...
    Module.prototype.getFinancialReport = function (params, callback) {
      ...
      SELECT \
        ...
        ifnull(SUM(CASE WHEN total > 0 THEN total ELSE 0 END), 0) as totalSales, \
        ifnull(SUM(CASE WHEN total < 0 THEN total ELSE 0 END), 0) as totalRefunds, \
        count(idbill) as transactions, \
        ifnull(SUM(CASE WHEN total > 0 THEN 1 ELSE 0 END), 0) as sales, \
        ifnull(SUM(CASE WHEN total < 0 THEN 1 ELSE 0 END), 0) as refunds \
      From bill \
      WHERE (closedDate BETWEEN '" + params.fromDate + "' AND '" + params.toDate + "' ) \
      ...
      From bill \
      WHERE (closedDate BETWEEN '" + params.fromDate + "' AND '" + params.toDate + "' ) \
      AND category = 'membership'", function (err, rows) {
                cleanup();
